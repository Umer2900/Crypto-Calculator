...
import { AlertCircle, ArrowUpRight, Calculator, DollarSign, Info, Percent, RefreshCcw, TrendingUp, Zap, } from 'lucide-react';
...
import type { IFuturesState, IStrategyState, ITaxState } from '../types';
...
const MultipleStrategy = ({ state, setState }: { state: IStrategyState; setState: any }) => {
  ...
  const getCommentary = () => {
    ...
    if (P_cur < P_old)
      return {
        text: 'Prices are lower than your entry. Averaging down will significantly reduce your break-even point.',
        color: 'text-blue-400',
      };
    if (P_cur > P_old)
      return {
        text: 'Prices are higher than your entry. Averaging up will increase your position size but also your average entry price.',
        color: 'text-red-400',
      };
    return {
      text: 'Current price matches your old entry. No change in average price if you add more.',
  ...
  return (
    <div className="grid lg:grid-cols-[1fr_2fr] gap-8">
      <div className="flex flex-col gap-6">
        <Card>
          <h3 className="text-xl font-bold bg-gradient-to-r from-red-400 to-blue-500 bg-clip-text text-transparent mb-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <Card className="bg-blue-500/5 border-blue-500/20">
              <div className="flex gap-3">
                ...
                <p className={cn('text-sm leading-relaxed', commentary.color)}>{commentary.text}</p>
              </div>
            </Card>
          </motion.div>
        )}
...
const FuturesCalculator = ({ state, setState }: { state: IFuturesState; setState: any }) => {
  ...
  const results = useMemo(() => {
    ...
    const liqPrice = position === 'long' ? P * (1 - 100 / L / 100) : P * (1 + 100 / L / 100);

    const feeIn = I * 0.005 * 1.18;

    const items = targets.map((p) => {
      const move = p / L;
      const targetPrice = position === 'long' ? P * (1 + move / 100) : P * (1 - move / 100);
      ...
      <div className="relative max-w-6xl mx-auto px-4 py-8 md:py-12">
        ...
        <main>
          <AnimatePresence mode="wait">
            ...
            >
              {activeTab === 'strategy' && (
                <MultipleStrategy state={strategyState} setState={setStrategyState} />
              )}
              {activeTab === 'tax' && <TaxCalculator state={taxState} setState={setTaxState} />}
              {activeTab === 'futures' && (
                <FuturesCalculator state={futuresState} setState={setFuturesState} />
              )}
            </motion.div>
          </AnimatePresence>
        </main>
        ...
        <footer className="mt-20 pt-8 border-t border-slate-800/50 text-center text-slate-500 text-sm">
          ...
          <div className="mt-2 flex items-center justify-center gap-4">
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-emerald-500" /> Real-time logic
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-blue-500" /> Secure & Private
            </span>
          </div>
        </footer>
      </div>
